...
public class MealStatListHandler extends ValueListHandler {
		...
		sqlShow.append("select csipro.ReferCampaignID subname,count(distinct csipro.csiid) amount");
		sqlTable.append(" from T_CSICustProductInfo csipro,T_CustServiceInteraction csi,T_Campaign camp");
		sqlWhere.append(" where csipro.CSIID = csi.ID and csipro.ReferCampaignID = camp.CampaignID" +
				" and camp.CampaignType ='" +CommonConstDefinition.CAMPAIGNCAMPAIGNTYPE_OPEN
				//+CommonConstDefinition.CAMPAIGNCAMPAIGNTYPE_GROUPBARGAIN+"','"
				//+CommonConstDefinition.CAMPAIGNCAMPAIGNTYPE_GROUPBARGAIN_OR_OPEN+"')"
				+"' and csi.Status='"+
		...
		sqlGroup.append(" group by csipro.ReferCampaignID");
