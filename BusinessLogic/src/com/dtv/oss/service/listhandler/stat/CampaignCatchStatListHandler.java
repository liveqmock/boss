...
public class CampaignCatchStatListHandler extends ValueListHandler {
		...
		public CampaignCatchStatListHandler(){
		...
		sqlShow.append("select cc.CampaignID subname,count(distinct cc.csiid) amount");
		sqlTable.append(" from t_customercampaign cc,T_CustServiceInteraction csi,T_Campaign camp");
		sqlWhere.append(" where cc.CSIID = csi.ID and cc.CampaignID = camp.CampaignID" +
				" and camp.CampaignType ='" +CommonConstDefinition.CAMPAIGNCAMPAIGNTYPE_NORMAL+
				"' and csi.Status='"+
		...
		sqlGroup.append(" group by cc.CampaignID");
		...
		if (dto.getSpareTime3() != null)
		{
			...
			sqlWhere.append(" and cc.DateFrom>=to_timestamp('").append(
		...
		if (dto.getSpareTime4() != null)
		{
			...
			sqlWhere.append(" and cc.DateTo<=to_timestamp('").append(
