...
%>
 ...
 <table align="center" width="100%" border="0" cellpadding="4" cellspacing="1" class="list_bg"> 	
    <tr class="list_bg2">
 	   	 <td valign="middle" align="right" width="33%">支付金额总计:
 	   	 	 <input type="text" style="text-align:right" size="10" name="generalPayTotal" value="0.0" readonly class="textgray">
 	   	 </td>
 	   	 <td valign="middle" align="right" width="33%">预存金额总计:
 	   	 	 <input type="text" style="text-align:right" size="10" name="forcePayTotal" value="0.0" readonly class="textgray">
 	   	 </td>   	 
 	   	 <td valign="middle" align="right" width="34%">支付金额总计:
 	   	 	 <input type="text" style="text-align:right" size="10" name="acctPaySum" value="0.0" readonly class="textgray">
 	   	 </td>   	 		
 	  </tr>
 </table>
 <table align="center" width="100%" border="0" cellpadding="4" cellspacing="1" class="list_bg"> 	
      ...
      <tr class="list_head">
        ...
        <td class="list_head" width="15%" align="center">费用支付金额</td>
        <td class="list_head" width="15%" align="center">预存金额</td>
  ...
  <%
       ...
       double intpayTotal =0;
       double intprepayTotal =0;
  %>
      ...
      <tbl:printcsstr style1="list_bg1" style2="list_bg2" overStyle="list_over" > 
        ...
        %>
           ...
           <%
                  ...
                  intprepayTotal =intprepayTotal +paymentDto.getAmount();
           %>
              <td align="center">0.00</td>
              <td align="center"><tbl:write name="oneline" property="Amount" /></td>
           <% 
 ...
 <input type="hidden" name="intprepayTotal" value="<%=intprepayTotal%>" />
 </table>
             ...
             if ( i % 2 ==0 ){
	           ...
	           payTableBody =payTableBody +"<tr id='"+i+"'  class='"+styletr+"' >"    	
	                        ...
	                        +"  <td align='center' width='30%'><input type='hidden' name='pay_realMOP' value='"+pay_realMOP[i]+"'>"+strMopName1+"</td>" 							 							
 	                        ...
 	                        +"  <td align='center' width='15%'><input type='text' name='pay_realpay' size='12' style='text-align:right' value="+pay_realPay[i]+" readonly='true' class='textgray'></td>"				
 	                        +"  <td align='center' width='15%'><input type='text' name='pay_realprepay' size='12' style='text-align:right' value="+pay_realPrepay[i]+" readonly='true'  class='textgray'></td>"	
    ...
    %>
    	<tr id='<%=pay_rownum%>'  class='<%=styletr%>' >
    	  ...
    	  <%
           ...
           if ((pay_realPay !=null && pay_realPay.length > 0) || 
        ...
        <td align="center" width="15%"><input name="pay_realpay" type="text" size="12"  style="text-align:right" value="0.0" class="text" onBlur='javaScript:pay_repeatProgression();' ></td>
        <td align="center" width="15%"><input name="pay_realprepay" type="text" size="12" style="text-align:right" value="0.0"  class="text" onBlur='javaScript:pay_repeatProgression();' ></td>
        ...
        <%
           ...
           double defaultForceFeeTotal =(request.getAttribute("defaultForceFeeTotal")==null) ? 0 : ((Double)request.getAttribute("defaultForceFeeTotal")).doubleValue();
           double default_forceFee = defaultForceFeeTotal-intprepayTotal ;
        %>
        <td align="center" width="15%"><input name="pay_realpay" type="text" size="12"  style="text-align:right" value="<%=((int)(default_generalFee*100))/100f %>" class="text" onBlur='javaScript:pay_repeatProgression();' ></td>
        <td align="center" width="15%"><input name="pay_realprepay" type="text" size="12" style="text-align:right" value="<%=((int)(default_forceFee*100))/100f %>"  class="text" onBlur='javaScript:pay_repeatProgression();' ></td>
        <td align="center" width="10%"><input type='button' name='addbutton' value='增加' onClick='javascript:add_pay();' class='button'></td> 
       <%} %>
      </tr>
...
<script language=javascript>
   ...
   function add_pay(){
       ...
       tempBody=tempBody+"<input type='hidden' name='pay_realtrid' value='"+pay_rownum+"'>"
	     ...
	     tempBody=tempBody+"<td align='center' width='15%'><input name='pay_realpay' type='text' size='12' style='text-align:right' value='"+ pay_pay[column].value +"' readonly='true' class='textgray'></td>";						//支付金额
 	     tempBody=tempBody+"<td align='center' width='15%'><input name='pay_realprepay' type='text' size='12' style='text-align:right' value='"+ pay_prepay[column].value +"' readonly='true'  class='textgray'></td>";						//预付金额
	    ...
	    pay_tableBody=pay_tableBody+"<td align='center' width='15%'><input name='pay_realpay' type='text' size='12' style='text-align:right' value='0.0'  class='text' onBlur='javaScript:pay_repeatProgression();'></td>";			//支付金额
 	    pay_tableBody=pay_tableBody+"<td align='center' width='15%'><input name='pay_realprepay' type='text' size='12' style='text-align:right' value='0.0'  class='text' onBlur='javaScript:pay_repeatProgression();'></td>";		//预付金额
   ...
   function pay_repeatProgression(){  
       ...
       var pay_tempPrepayTotal =parseFloat(document.all("intprepayTotal").value);
       init();
       if (pay_pay) {
           for (i=0; i<pay_pay.length;i++){
                  ...
                  pay_tempPrepayTotal =Math.floor(pay_tempPrepayTotal*100+parseFloat(pay_prepay[i].value)*100+0.01)/100;
               }
           }
       }
        
       document.all("generalPayTotal").value=pay_tempPayTotal;
	     document.all("forcePayTotal").value=pay_tempPrepayTotal;
	     document.all("acctPaySum").value =Math.floor(pay_tempPayTotal*100 +pay_tempPrepayTotal*100+0.01)/100;
   }
  
   function init(){
   	  pay_pay =document.getElementsByName("pay_realpay");
      pay_prepay =document.getElementsByName("pay_realprepay");
   ...
   function del_payText(rownum) {
      ...
      if (pay_rownum % 2 ==0 ){
	    ...
	    tempBody=tempBody+"<td align='center' width='15%'><input name='pay_realpay' type='text' size='12' style='text-align:right' value='"+pay_pay[column].value+"'  class='text' onBlur='javaScript:pay_repeatProgression();'></td>";						//支付金额
 	    tempBody=tempBody+"<td align='center' width='15%'><input name='pay_realprepay' type='text' size='12' style='text-align:right' value='"+pay_prepay[column].value +"'  class='text' onBlur='javaScript:pay_repeatProgression();'></td>";						//预付金额
   ...
   function check_Pay(rowncolumn){
	    ...
	    if (!check_Float(pay_pay[rowncolumn], true, "费用支付金额"))
	        return false;
	    if (check_Blank(pay_prepay[rowncolumn], true, "预付金额"))
	        return false;
	    if (!check_Float(pay_prepay[rowncolumn], true, "预付金额"))
	        return false; 
	     
	    if (pay_prepay[rowncolumn].value*1.0 <0){
            alert("预付金额不能为负数");
            pay_prepay[rowncolumn].focus();
            return false;
      }
      ...
      if (pay_mopid ==10 || pay_mopid ==11){
      	  if (pay_prepay[rowncolumn].value*1.0 !=0){
      	  	  alert("采用预存抵扣方式不能有预存金额！");
      	  	  pay_prepay[rowncolumn].value =0;
      	  	  pay_prepay[rowncolumn].focus();
      	  	  return false;
      	  }
      	  if (pay_pay[rowncolumn].value*1.0 <0){
