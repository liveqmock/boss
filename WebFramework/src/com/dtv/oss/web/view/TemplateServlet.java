...
public class TemplateServlet extends HttpServlet {
    ...
    private HashMap tableResources;

    //Initialize global variables
    public void init(ServletConfig config) throws ServletException {
        System.out.println("init TemplateServlet Class");
        ...
        //for orderby of query
       URL tableMappings = null;
        try {
            tableMappings = context.getResource("/WEB-INF/table-definition.xml");
            TableDefDAO dao = new TableDefDAO(tableMappings);
            tableResources = dao.getTableResources();

        } catch (java.net.MalformedURLException ex) {
            System.err.println("QueryOrderServlet: initializing table difinition malformed URL exception: " + ex);
        }

        context.setAttribute(WebKeys.TABLE_DEFINITION , tableResources);

    }

    private void initScreens(ServletContext context, String language) {
        URL screenDefinitionURL = null;
        String bossFlag=context.getInitParameter("boss.type.flag");
        if (bossFlag==null) bossFlag = "";

        String resourceURL = "/WEB-INF/screendefinitions_";
        if (!bossFlag.trim().equals("")) resourceURL += bossFlag + "_";
        resourceURL += language + ".xml";

        try {
            screenDefinitionURL = context.getResource(resourceURL);
        } catch (java.net.MalformedURLException ex) {
        ...
        if (screenDefinitionURL != null) {
            ...
            if (screenDefinitions != null) {
                ...
                System.err.println("Template Servlet Error Loading Screen Definitions: Confirm that file at URL "+ resourceURL +" contains the screen definitions");
            }
        } else {
            System.err.println("Template Servlet Error Loading Screen Definitions: URL "+ resourceURL +" not found");
        }
    }
    ...
    public void process (HttpServletRequest request,
        ...
        if (lastPathSeparator != -1 && lastDot != -1 && lastDot > lastPathSeparator) {
            screenName = selectedUrl.substring(lastPathSeparator + 1, lastDot);
        }
        ...
        if (screenName.equals(PREVIOUS_SCREEN)) {
            ...
            //if (request.getSession().getAttribute(WebKeys.PREVIOUS_SCREEN)!=null)
            //    request.getSession().setAttribute(WebKeys.BACK_SCREEN, request.getSession().getAttribute(WebKeys.PREVIOUS_SCREEN));
