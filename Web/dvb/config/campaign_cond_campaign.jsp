...
function add_submit(){
	document.frmOther.action="add_cond_campaign.screen";
	document.frmOther.submit();
}

function view_submit(seqNo){
	document.frmOther.action="campaign_cond_view.screen?txtSeqNo="+seqNo;
...
function delete_object(strID){
    if( confirm("确定要删除该记录吗?") ){
        document.frmOther.action="delete_campaign_cond.do?txtSeqNo="+strID;
        document.frmOther.txtActionType.value="deletecampaign";
...
<table width="98%"  border="0" align="center" cellpadding="5" cellspacing="1" class="list_bg">
  <tr class="list_head">
    ...
    <td class="list_head">新增市场活动</td>
    <td class="list_head">全选标志</td>
     <td class="list_head">活动数量</td>
   ...
   <%
        ...
        List  dtoList =   Postern.getCampaignCondCampaignDTOList(campaignID);
        
        if (dtoList!=null){
          Iterator ite = dtoList.iterator();
          while (ite.hasNext()){ 
          CampaignCondCampaignDTO condCamDto= (CampaignCondCampaignDTO) ite.next();
          pageContext.setAttribute("oneline",condCamDto);
...
<tbl:printcsstr style1="list_bg1" style2="list_bg2" overStyle="list_over" >
    ...
    <td><tbl:write name="oneline" property="campaignIDList"/> </td>
    <td><d:getcmnname typeName="SET_G_YESNOFLAG" match="oneline:hasAllFlag" /></td> 
    <td><tbl:write name="oneline" property="campaignNum"/> </td>
