...
function add_submit(){
	document.frmOther.action="add_cond_package.screen";
	document.frmOther.submit();
}

function view_submit(seqNo){
	document.frmOther.action="package_cond_view.screen?txtSeqNo="+seqNo;
...
function delete_object(strID){
    if( confirm("确定要删除该记录吗?") ){
        document.frmOther.action="delete_package_cond.do?txtSeqNo="+strID;
        document.frmOther.txtActionType.value="deletepackage";
...
<table width="98%"  border="0" align="center" cellpadding="5" cellspacing="1" class="list_bg">
  <tr class="list_head">
    ...
    <td class="list_head">新增产品包</td>
    <td class="list_head">全选标志</td>
     <td class="list_head">产品包数量</td>
    <td class="list_head">新购买标志</td>
     <td class="list_head">存在标志</td>
     <td class="list_head">操作</td>  
  </tr> 
   <%
         
        List  dtoList =   Postern.getCampaignCondPackageDTOList(campaignID);
        String  PackageNames = "";
        if (dtoList!=null){
          Iterator ite = dtoList.iterator();
          while (ite.hasNext()){ 
          CampaignCondPackageDTO condProdDto= (CampaignCondPackageDTO) ite.next();
          pageContext.setAttribute("oneline",condProdDto);
          PackageNames = Postern.getPackageNameByPackageIDs(condProdDto.getPackageIdList());
...
<tbl:printcsstr style1="list_bg1" style2="list_bg2" overStyle="list_over" >
    ...
    <td><tbl:write name="oneline" property="packageNum"/> </td>
    <td><d:getcmnname typeName="SET_G_YESNOFLAG" match="oneline:newPurchaseFlag" /></td>  
