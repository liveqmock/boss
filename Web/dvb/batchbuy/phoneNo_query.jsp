...
function choose_confirm(){
   ...
   var itemId='';
	if(document.frmPost.signradio.length ==2)
	{
	   if (document.frmPost.signradio[1].checked){    
	       phoneNo =document.frmPost.phoneno[1].value;
	       itemId =document.frmPost.itemID[1].value;
	       check_flag =true;   
		}
	}
	else
	{
		for (i=1 ;i< document.frmPost.signradio.length ;i++){
	       if (document.frmPost.signradio[i].checked){
	           ...
	           phoneNo =document.frmPost.phoneno[i].value;
	           itemId =document.frmPost.itemID[i].value;
   ...
   if (check_flag==false){
      ...
      return ;
   		...
   		window.returnValue=phoneNo+','+itemId;
      window.close();
   	}
}
</Script>
 ...
 <form name="frmPost" method="post" action="phoneNo_query_batchbuy.do" >
