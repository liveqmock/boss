...
function query_submit(){
	document.frmPost.action="forcedeposit_futureright_for_finace_stat.do";
	if(checkDate()){ 
...
<form name="frmPost" method="post" action="forcedeposit_futureright_for_finace_stat.do">
	...
	<tr align="center">
	  <td class="list_bg1"><table  border="0" cellspacing="0" cellpadding="0">
		  <tr>
			...
			<pri:authorized name="forcedeposit_futureright_for_finace_stat_excel.do" >
			<td width="22" height="20">
			<td width="11" height="20"><img src="img/button_l.gif" width="11" height="20"></td>
			<td><input name="Submit" type="button" class="button" value="保存结果" onclick="javascript:save_query_result('押金和期权统计',true)"></td>
			<td width="22" height="20"><img src="img/button_r.gif" width="22" height="20"></td>
			</pri:authorized>
		  </tr>
	  </table></td>
...
<%
	...
	Map mapSecSub = Postern.getHashKeyValueByName("SET_F_FUTURERIGHTSTATUS");
	...
	if (mapShow != null)
	{
		for (int i=0;i<colCount;i++)
		{
				...
				for(int j=0;j<secSubCount;j++)
				{
					...
					%>
					<td align="center" class="t12" nowrap><%=WebUtil.bigDecimalFormat(dValue)%></td>
					<%
				} 
		}
	}
%>
		<td align="center" class="t12"><%=WebUtil.bigDecimalFormat(dAllValue)%></td>
  </tbl:printcsstr>
</lgc:bloop>  
	<tr class="trline" > 
...
<%
	...
	if (mapShow != null)
	{
		for (int i=0;i<colCount;i++)
		{
			for(int j=0;j<secSubCount;j++)
			{
				double dValue = allValue[i][j];
				dSumAll = dSumAll + dValue;
				%>
				<td align="center" class="t12"> <%=WebUtil.bigDecimalFormat(dValue)%> </td>
				<%
			}
		}
	}
%>					
	<td align="center" class="t12"> <%=WebUtil.bigDecimalFormat(dSumAll)%> </td>
  </tr>
    <tr>
    <td colspan="<%=WebUtil.bigDecimalFormat(colCount*secSubCount+2)%>" class="list_foot"></td>
  </tr>
</table>	
</div>	
</rs:hasresultset>
