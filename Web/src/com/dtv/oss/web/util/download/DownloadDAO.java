...
public class DownloadDAO {
	...
	private int executeCountQuery(String sql,Object para) {
		...
		try {
			...
			e.printStackTrace();
			LogUtility.log(DownloadDAO.class, LogLevel.WARN, "executeCountQuery", e);
			i = -1;
		} finally {
			if (rs != null) {
				try {
					...
					LogUtility.log(DownloadDAO.class, LogLevel.WARN, "executeCountQuery",
							e);
				}
			}
			if (stmt != null) {
...
//				int columnType=meta.getColumnType(i);
	...
	public static DownloadDAO init(String sql,Object para) throws WebActionException{
		if(sql==null||"".equals(sql)){
			throw new WebActionException("没有有效查询条件.");
		}
		DownloadDAO qu=new DownloadDAO();
		qu.setQuerySql(sql);
		...
		LogUtility.log(DownloadDAO.class, LogLevel.DEBUG, "DownloadDAO★★★★总记录数★★★★"+qu.getCount());
...
//		System.out.println("to::"+qu.getTo());
	...
	private Map getMetaMap() throws SQLException{
		Map metaMap=new HashMap();
		ResultSetMetaData meta= resultSet.getMetaData();
		for(int i=1;i<=meta.getColumnCount();i++){
			String label=meta.getColumnLabel(i);
			int type=meta.getColumnType(i);
			metaMap.put(label, new Integer(type));
		}
		return metaMap;
	}
	...
	public Map next() throws SQLException {
		...
		for (Iterator it = columnLabel.iterator(); it.hasNext();) {
			...
			if(java.sql.Types.NUMERIC == ((Integer)metaMap.get(columnLabel)).intValue() 
					|| java.sql.Types.TINYINT == ((Integer)metaMap.get(columnLabel)).intValue() 
					|| java.sql.Types.SMALLINT == ((Integer)metaMap.get(columnLabel)).intValue() 
					|| java.sql.Types.INTEGER == ((Integer)metaMap.get(columnLabel)).intValue() 
					|| java.sql.Types.BIGINT == ((Integer)metaMap.get(columnLabel)).intValue() 
					|| java.sql.Types.FLOAT == ((Integer)metaMap.get(columnLabel)).intValue() 
					|| java.sql.Types.DOUBLE == ((Integer)metaMap.get(columnLabel)).intValue() 
					|| java.sql.Types.DECIMAL == ((Integer)metaMap.get(columnLabel)).intValue() 
				)
				if(val == null)
...
//			if(val instanceof oracle.sql.DATE){}
	...
	protected void finalize()throws Throwable{
		...
		try {
			...
			LogUtility.log(DownloadDAO.class, LogLevel.WARN,e.getMessage());
		}
	}
}
