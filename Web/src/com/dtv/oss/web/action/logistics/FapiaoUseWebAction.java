 ...
 * 发票领用
 */
public class FapiaoUseWebAction extends GeneralWebAction {
	...
	protected EJBEvent encapsulateData(HttpServletRequest request)
		...
		if (!WebUtil.StringHaveContent(request.getParameter("txtOutOrgID")))
			throw new WebActionException("参数错误:领用组织未知!");
		...
		// 目标地址ID :领用到组织机构
		if(WebUtil.StringHaveContent(request.getParameter("txtOutOrgID")))
			faPiaoDto.setAddressID(WebUtil.StringToInt(request.getParameter("txtOutOrgID")));
		...
		theEvent.setActionType(LogisticsEJBEvent.FAPIAO_USE);
