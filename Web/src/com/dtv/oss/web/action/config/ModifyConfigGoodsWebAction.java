 ...
 */
public class ModifyConfigGoodsWebAction extends GeneralWebAction {
   ...
   public void doStart(HttpServletRequest request)
    ...
    protected EJBEvent encapsulateData(HttpServletRequest request) throws WebActionException {
        ...
        if("create".equals(request.getParameter("Action")))
			event.setActionType(EJBEvent.CONFIG_GOODS_CREATE);
	    if("update".equals(request.getParameter("Action"))){
	    	dto.setDtLastmod(WebUtil.StringToTimestamp(request.getParameter("txtDtLastmod")));
			event.setActionType(EJBEvent.CONFIG_GOODS_EDIT);
	    }
    	...
    	if (WebUtil.StringHaveContent(request.getParameter("txtAmount")))
            dto.setAmount(WebUtil.StringToInt(request.getParameter("txtAmount")));
    	if (WebUtil.StringHaveContent(request.getParameter("txtID")))
            dto.setId(WebUtil.StringToInt(request.getParameter("txtID")));
           event.setGoodsDto(dto);
