...
//20140222 添加发票
public class AddFapiaoSerialNoWebAction extends GeneralWebAction {
...
//		LogUtility.log(this.getClass(), LogLevel.DEBUG, "txtAction"+action);
		...
		for(int index=0;index<arrRowId.length;index++){
            ...
            if(fapiao_serialno!=null){
                acctDto.setFapiaoSerialNo(fapiao_serialno);
            }
            if(fapiao_haoma!=null){
                acctDto.setFapiaoHaoma(fapiao_haoma);
            }
            //end 20140222
       ...
       //add 20140222 添加发票信息
            if(request.getParameter("fapiao_serialno")!=null){
                feeInfo.setFapiaoSerialNo(request.getParameter("fapiao_serialno"));
            }
        //end 20140222
