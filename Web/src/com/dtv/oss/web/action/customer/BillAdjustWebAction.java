...
public class BillAdjustWebAction extends GeneralWebAction {
	...
	private int getAdjustType(HttpServletRequest request) throws WebActionException{
		...
		if(CommonKeys.ADJUST_REFERCODETYPE_D.equals(adjustType)){
			return AccountAdjustEJBEvent.BILL_ADJUST_PREPAYMENTDEDUCTION;
		}else if(CommonKeys.ADJUST_REFERCODETYPE_F.equals(adjustType)){
			return AccountAdjustEJBEvent.BILL_ADJUST_FEE;
		}else if(CommonKeys.ADJUST_REFERCODETYPE_C.equals(adjustType)){
			return AccountAdjustEJBEvent.BILL_ADJUST_PAYMENT;
		}else if(CommonKeys.ADJUST_REFERCODETYPE_P.equals(adjustType)){
			return AccountAdjustEJBEvent.BILL_ADJUST_PREPAYMENT;
		}else{
            throw new WebActionException("未知类型的调帐!");
		}
	}
	...
	protected EJBEvent encapsulateData(HttpServletRequest request)
		...
		if(AccountAdjustEJBEvent.BILL_ADJUST_PREPAYMENTDEDUCTION==adjustType){
		}else if(AccountAdjustEJBEvent.BILL_ADJUST_FEE==adjustType){
			event.setAccountItemDTO(encapsulateAccountItemDTO(request));
		}else if(AccountAdjustEJBEvent.BILL_ADJUST_PAYMENT==adjustType){
			event.setPaymentRecordDTO(encapsulatePaymentRecordDTO(request));
		}else if(AccountAdjustEJBEvent.BILL_ADJUST_PREPAYMENT==adjustType){
			event.setPaymentRecordDTO(encapsulatePaymentRecordDTO(request));
		}
	...
	private AccountItemDTO encapsulateAccountItemDTO(HttpServletRequest request) throws WebActionException{
        ...
        String invoiceNo=request.getParameter("txtInvoiceNo");
        if(WebUtil.StringHaveContent(invoiceNo)){
        	acctItemDto.setInvoiceNO(WebUtil.StringToInt(invoiceNo));
        }else{
        	throw new WebActionException("无效的帐单号!");
        }
	...
	private PaymentRecordDTO encapsulatePaymentRecordDTO(HttpServletRequest request) throws WebActionException{
        ...
        String invoiceNo=request.getParameter("txtInvoiceNo");
        if(WebUtil.StringHaveContent(invoiceNo)){
//        	paymentDto.setPaidTo(WebUtil.StringToInt(invoiceNo));
        	paymentDto.setInvoiceNo(WebUtil.StringToInt(invoiceNo));
        }else{
        	throw new WebActionException("无效的帐单号!");
        }
